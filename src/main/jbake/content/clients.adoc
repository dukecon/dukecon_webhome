:filename: main/jbake/content/clients.adoc
:jbake-type: page
:jbake-title: Clients
:jbake-status: published
:imagesdir: ./images/
:linkattrs: true

= Clients


== Conferences using DukeCon

include::config.adoc[]
{improve}


== Native mobile

=== Android

[cols=2, frame=none,grid=none]
|===
|https://github.com/dukecon/dukecon_android[image:JavaLand-Android-2018.png[Javaland 2018,width=200]]
|https://github.com/dukecon/dukecon_android[image:Apex-Android-2018.png[Apex 2018,width=200]]
|===

=== SailfishOS

[cols=2, frame=none,grid=none]
|===
|https://github.com/andywuest/harbour-sailcon[image:Sailfish_Screenshot1.png[Screenshot 1,width=200]]
|https://github.com/andywuest/harbour-sailcon[image:Sailfish_Screenshot2.png[Screenshot 2,width=200]]
|===


=== Multiplatform

* https://github.com/dukecon/dukecon_mobile[Kotlin multiplatfrom native client Android, iOS]

==== Getting started

* https://dukecon.github.io/dukecon_mobile/workshops/javaland2020.html[JavaLand 2020 - Kotlin multiplatfrom workshop]

